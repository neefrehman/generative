...
const sketch = (p: p5) => {
  ...
  class Ball {
    ...
    update() {
      ...
      this.x =
        mouseLocation.x + p.map(p.noise(this.xOff), 0, 1, -p.width / 2, p.width / 2);
      this.y =
        mouseLocation.y +
    ...
    collides(sibling: { x: number; y: number; z: number; r: number }) {
      const d = p.dist(this.x, this.y, this.z, sibling.x, sibling.y, sibling.z);
    ...
    if (mouseHasMoved) {
      target =
        lastMouseMovement < p.frameCount - 100
          ? p.createVector(p.width / 2, p.height / 2, shortestDimension / 2)

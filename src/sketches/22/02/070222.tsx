...
const sketch: Canvas2DSetupFn = ({ width, height, ctx }) => {
  ...
  const repellors = [...Array(30)].map(() => inSquare(SCALE, SCALE));

  const alphaCap = inRange(0.2, 1.5);
  ...
  return ({ frameCount, mousePosition }) => {
    ...
    points.forEach(({ position: [x, y], color }) => {
      ...
      const u = lerp(x, closestRepellor[0], inRange(alphaCap));
      const v = lerp(y, closestRepellor[1], inRange(alphaCap));
...
const S060222 = () => (
  <Canvas2DRenderer sketch={sketch} settings={{ animationSettings: { fps: 24 } }} />
);

...
const sketch: Canvas2DSetupFn = ({ width, height, ctx }) => {
  ...
  const BALL_COUNT = inRange(32, 38, { isInteger: true });
  ...
  let NEAREST_POINT_COUNT = 0;

  return ({ mouseIsIdle, mousePosition, mouseIsDown, frameCount }) => {
    ...
    if (isCreative) {
      ...
      if (mouseIsDown) {
        NEAREST_POINT_COUNT += NEAREST_POINT_COUNT < 64 ? 2 : 0;
      } else {
        NEAREST_POINT_COUNT = lerp(0, 36, Math.min(frameCount / 20, 1));
      }
      ...
      balls.forEach(ball => {
        ball.update(ctx, {
          ...
          mouseOffset: lerp(0, 320, Math.min(frameCount / 20, 1)),
        });

        const nearestPoints = ball.getNearestPoints(points, NEAREST_POINT_COUNT);

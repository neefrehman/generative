...
const sketch: ShaderSetupFn = ({ width, height, aspect }) => ({
  ...
  frag: glsl`
        ...
        // https://twitter.com/smjtyazdi/status/1484828390104485896?s=21
        float slice(vec3 pos, float thickness) {
            float result = 1000.0
            
            for (float j = -1.0; j <= 1.0; j += 1.0) {
                vec3 np = pos;
                np.z = round(np.z / thickness + j) * thickness;
                float sd = sdf(np);
                np.z = clamp(pos.z, np.z - thickness / 2.0, np.z + thickness / 2.0);
                sd = length(vec2(max(0.0, sd), np.z - pos.z));
                res = min(res, sd);
            }
